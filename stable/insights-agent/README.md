...
`global.images.imagePullSecrests` | Pull secrets to be added to every pod (e.g. `[{"name": "my-secret"}]`) | null
...
`uploader.imagePullSecret` | **deprecated**: use `global.images.imagePullSecrets` instead. A pull secret for a private uploader image
...
`cronjobs.imagePullSecret` | **deprecated**: use `global.images.imagePullSecrets` instead. A pull secret for cronjob images
