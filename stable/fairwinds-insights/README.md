...
| postgresql.auth.existingMigrationSecret | string | `"fwinsights-postgresql-migration"` | Secret for the migration user when `migrationUsername` differs from `username`. Ignored otherwise (migration Job uses `existingSecret`). Not created by `auth.externalSecret`; provision separately when using Vault/ESO. |
