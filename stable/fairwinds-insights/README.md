...
| postgresql.auth | object | `{"database":"fairwinds_insights","existingSecret":"fwinsights-postgresql","existingSuperUserSecret":"fwinsights-postgresql-superuser","externalSecret":{"data":[{"remoteRef":{"key":"insights/postgres","property":"postgresql_password"},"secretKey":"postgresql-password"},{"remoteRef":{"key":"insights/postgres","property":"postgresql_password"},"secretKey":"password"},{"remoteRef":{"key":"insights/postgres","property":"readonly_password"},"secretKey":"readonly-password"},{"remoteRef":{"key":"insights/postgres","property":"timescale_password"},"secretKey":"timescale-password"}],"enabled":false},"secretKeys":{"adminPasswordKey":"postgresql-password"},"username":"postgres"}` | Authentication configuration. When `auth.externalSecret.enabled` and not `ephemeral`, an ExternalSecret is created: set `auth.externalSecret.data` to the `spec.data` list (each entry `secretKey` + `remoteRef`); defaults target Vault KV `insights/postgres`. The chart pins `apiVersion`, refresh policy/interval, and ClusterSecretStore `fairwinds-vault-backend`. |
