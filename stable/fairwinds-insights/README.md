...
| gcpPricing | object | `{"credentialsMountPath":"","credentialsSecretKey":"","credentialsSecretName":"","credentialsSecretOptional":true,"enabled":false,"externalSecret":{"annotations":{},"create":false,"data":[],"name":"","refreshInterval":"1h","secretStoreRef":{"kind":"ClusterSecretStore","name":"fairwinds-vault-backend"}},"projectID":""}` | GCP Cloud Billing / Compute pricing for UpdateGCPCloudPricingWorkflow (general-worker). |
| gcpPricing.credentialsSecretName | string | `""` | Secret with the GCP SA JSON. Empty uses gcp-pricing-credentials |
| gcpPricing.credentialsSecretKey | string | `""` | Key in that Secret. Empty uses credentials.json |
| gcpPricing.credentialsMountPath | string | `""` | Mount path for the Secret. Empty uses /var/run/secrets/gcp |
| gcpPricing.credentialsSecretOptional | bool | `true` | If true, general-worker starts even when the credentials Secret is missing |
