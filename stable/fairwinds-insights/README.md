...
| cronjobs.partman-maintenance | object | `{"command":"partman_maintenance","schedule":"30 0 * * *","useMigrationCredentials":false}` | Options for pg_partman maintenance. useMigrationCredentials: connect as migrator and set POSTGRES_OWNER_ROLE so new partitions are owned by ownerRole (required after owner-role migration). |
