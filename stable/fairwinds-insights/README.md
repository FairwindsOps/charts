...
| resourcesRecommendationsCronjob.enabled | bool | `true` | Enable database cleanup true by default |
| resourcesRecommendationsCronjob.resources | object | `{"limits":{"cpu":"500m","memory":"1024Mi"},"requests":{"cpu":"80m","memory":"128Mi"}}` | Resources for the database cleanup job. |
| resourcesRecommendationsCronjob.schedules | list | `[{"cron":"0 0 * * *","interval":"1h","name":"resources-recommendations"}]` | CRON schedules for the database cleanup job. |
| resourcesRecommendationsCronjob.securityContext.runAsUser | int | `10324` | The user ID to run the database cleanup job under. |
