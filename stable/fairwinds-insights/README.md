...
| postgresql.auth.existingMigrationSecret | string | `""` | Secret for the migration user when `migrationUsername` differs from `username`. Empty with ephemeral: chart creates `fwinsights-postgresql-migration`. Required when split and not ephemeral. Same keys (`username` + `password`); there is no ExternalSecret block for this Secret. |
...
| timescale.auth.existingMigrationSecret | string | `""` | Secret for the migration user when `migrationUsername` differs from `postgresqlUsername`. Empty with ephemeral: chart creates `fwinsights-timescale-migration`. Required when split and not ephemeral. Same keys (`username` + `password`). |
...
| cronjobExecutor.image.tag | string | `"1.36.2"` |  |
...
| temporalDeploymentDefaults | object | `{"additionalEnv":[],"affinity":null,"annotations":{},"args":[],"hpa":{"enabled":true,"max":4,"metrics":[],"min":2},"image":{"repository":"","tag":""},"imagePullPolicy":"Always","initContainers":[],"keda":{"advanced":{"horizontalPodAutoscalerConfig":{"behavior":{"scaleDown":{"policies":[{"periodSeconds":120,"type":"Percent","value":50}],"stabilizationWindowSeconds":300},"scaleUp":{"policies":[{"periodSeconds":60,"type":"Percent","value":50},{"periodSeconds":60,"type":"Pods","value":1}],"selectPolicy":"Min","stabilizationWindowSeconds":120}}}},"cooldownPeriod":300,"enabled":false,"max":10,"min":1,"pollingInterval":30,"prometheus":{"activationThreshold":"5","ignoreNullValues":"true","query":"","serverAddress":"","threshold":"30"}},"nodeSelector":{},"pdb":{"enabled":false,"minAvailable":1},"podAnnotations":{},"priorityClassName":"","rbac":{"enabled":false,"serviceAccount":{"annotations":{}}},"resources":{"limits":{"cpu":"1000m","memory":"1024Mi"},"requests":{"cpu":"500m","memory":"512Mi"}},"securityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]},"privileged":false,"readOnlyRootFilesystem":true,"runAsNonRoot":true,"runAsUser":10324},"terminationGracePeriodSeconds":600,"tolerations":[],"topologySpreadConstraints":[],"useReadReplica":false,"volumeMounts":[{"mountPath":"/var/run/secrets/github","name":"secrets"}],"volumes":[{"name":"secrets","secret":{"optional":true,"secretName":"github-secrets"}}]}` | Default options for temporal deployments |
...
| gcpPricing | object | `{"credentialsMountPath":"/var/run/secrets/gcp","credentialsSecretKey":"credentials.json","enabled":false,"existingSecret":"","externalSecret":{"annotations":{},"create":false,"data":[],"name":"gcp-pricing-credentials-external","refreshInterval":"1h","secretStoreRef":{"kind":"ClusterSecretStore","name":"fairwinds-vault-backend"}},"projectID":""}` | GCP Cloud Billing / Compute pricing for UpdateGCPCloudPricingWorkflow (general-worker). Off by default. With no secret, ADC is used (DefaultTokenSource) and GOOGLE_APPLICATION_CREDENTIALS is not set. |
| gcpPricing.enabled | bool | `false` | Set ENABLE_GCP_PRICING on general-worker |
| gcpPricing.projectID | string | `""` | GCP_PROJECT_ID for Compute machineTypes. Optional; the activity skips if unset. |
| gcpPricing.existingSecret | string | `""` | Existing Secret with the SA JSON. Empty: do not mount (unless externalSecret.create). |
| gcpPricing.credentialsSecretKey | string | `"credentials.json"` | Key in that Secret |
| gcpPricing.credentialsMountPath | string | `"/var/run/secrets/gcp"` | Mount path for that Secret |
| gcpPricing.externalSecret.create | bool | `false` | Create an ExternalSecret for the SA JSON |
| gcpPricing.externalSecret.name | string | `"gcp-pricing-credentials-external"` | ExternalSecret and the Secret it creates |
