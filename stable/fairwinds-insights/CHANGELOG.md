...
## 10.4.1
* Bumped `insights-api` to `18.4.52`
...
## 10.0.0
* **BREAKING:** PostgreSQL and Timescale share one secret-ownership model (Existing / External / Chart-managed). `existing*` defaults are empty; Secret keys are fixed (`password`, not `postgresql-password` / `timescale-password` / `secretKeys`); Timescale is not coupled to the PostgreSQL Secret; split migration is a second Secret (Existing, External via `migrationExternalSecret`, or Chart-managed); `timescale.password` / `superuserpassword` move under `timescale.auth`; `timescale.secrets` is removed; ExternalSecret `create: true` wires the app to `fwinsights-*`.
* Split migration External mode: `postgresql.auth.migrationExternalSecret` / `timescale.auth.migrationExternalSecret` (same exclusive rules as the app Secret; requires `migrationUsername` to differ).
...
## 9.21.6
* Bumped `insights-api` to `18.4.18`
...
## 9.14.5
* Bumped `insights-api` to `18.4.3`

## 9.14.4
* Bumped `insights-api` to `18.4.2`
...
## 9.14.1
* Bumped `swaggerapi/swagger-ui` to `v5.32.11`

## 9.14.0
* Bumped `insights-api` to `18.4.0`

## 9.13.10
* Bumped `insights-api` to `18.3.77`

## 9.13.9
* Bumped `swaggerapi/swagger-ui` to `v5.32.10`
...
## 9.13.3
* Bumped `insights-api` to `18.3.72`

## 9.13.2
* Bumped `insights-api` to `18.3.70`

## 9.13.1
* Bumped `insights-api` to `18.3.69`
...
## 9.12.6
* Bumped `insights-api` to `18.3.67`

## 9.12.5
* Bumped `insights-api` to `18.3.66`

## 9.12.4
* Soften default KEDA scale-up for temporal workers, so scale-up ramps instead of jumping to max

## 9.12.3
* Bumped `insights-api` to `18.3.65`
...
* **External secrets:** `postgresql.auth.externalSecret` provisions the application secret only. When `migrationUsername` differs from `username`, provision `existingMigrationSecret` separately (manual Secret, second ExternalSecret, or your secret store).
