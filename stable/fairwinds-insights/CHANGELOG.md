...
## 6.3.0
* Add optional **[RustFS](https://charts.rustfs.com/)** subchart for **local and self-hosted** deployments as an **in-cluster MinIO replacement**: set `minio.install: false`, `rustfs.install: true`, and `reportStorage.strategy: rustfs` to use `REPORT_STORAGE_S3_*` (in-cluster endpoint defaults to `http://{release}-{rustfs.nameOverride}-svc:{port}`; see `values.yaml`). External RustFS in the same scenarios: keep `minio.install: false`, set `reportStorage.s3Endpoint` and `reportStorage.s3CredentialsSecret` (`accessKeyId` / `secretAccessKey`) with `rustfs.install: false`. MinIO remains the default (`strategy: minio`, `minio.install: true`). Template install fails if `reportStorage.strategy` is `rustfs` while `minio.install` is still true. When `rustfs.install` is true and `rustfs.createBucket` is true (default), a normal release Job (not a Helm hook) creates the report bucket; set `rustfs.createBucket: false` to skip that Job if you manage the bucket yourself. Use `helm install|upgrade --wait --wait-for-jobs` so that Job finishes before Helm succeeds, or the app may start before the bucket exists.
* Default `values.yaml` only pins `rustfs` settings that differ from the upstream chart for a single-node in-cluster broker (standalone, Ingress off, storage size, etc.); other keys still pass through (`helm show values rustfs/rustfs`). The create-bucket Job and `REPORT_STORAGE_S3_ENDPOINT` use port `9000` and `amazon/aws-cli:2.34.19` when `rustfs.service.endpoint.port` / `rustfs.bucketJob` are omitted.
* `reportStorage.strategy` may be `s3` instead of `rustfs` when using in-cluster or external S3-compatible storage (`rustfs.install` and/or `s3Endpoint`); the app treats both the same when `REPORT_STORAGE_S3_ENDPOINT` is set (Insights `pkg/files/s3.go`).
* With `rustfs.install: true` and the default `reportStorage.strategy: minio`, the chart now sets `REPORT_STORAGE_STRATEGY=rustfs` and `REPORT_STORAGE_S3_*` (no `MINIO_*`). Prefer `minio.install: false` so only one object-store subchart runs (install prints a NOTE if both are enabled).
